...
- [ ] 用户可以选择所有 Phase 1 初始 Prompt Pack intent。
- [ ] 每个 flow 都展示将包含的上下文说明。
- [ ] Prompt Pack 生成结构化 Markdown prompt。
- [ ] 默认上下文包含当前 Task、紧凑 Project 上下文、dependency summaries、recent progress snippets 和 Memory 入口。
- [ ] Whole-Project analysis 必须由用户显式触发，不作为默认行为。
- [ ] 用户可以 copy prompt。
- [ ] 用户可以 paste back 外部 LLM 输出。
- [ ] LLM 输出默认不会自动修改 Project、Task、Journal 或 Memory。
- [ ] 用户可以 discard 或保存 LLM 输出为 LLM note。
- [ ] 保存 LLM note 后可在 workspace 中恢复，并追加相应 Journal 事件。
- [ ] 有测试覆盖不同 intent 的 prompt 内容、上下文边界、copy/paste-back 状态和 LLM 输出非事实规则。
